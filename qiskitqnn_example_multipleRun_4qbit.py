"""
Qiskit example  QNN running 100 times with 4 qbits 200iter COBYLA optimizer
"""
...
def conv_layer(num_qubits, param_prefix):
    ...
    # Calculate required parameters based on number of qubits
    if num_qubits == 4:
        num_params = 9  # (0,1), (2,3), (1,2) pairs * 3 parameters each
    elif num_qubits == 2:
        num_params = 3  # Only one pair * 3 parameters
    else:
        num_params = num_qubits * 3  # Default case
    
    params = ParameterVector(param_prefix, length=num_params)
    
    # First layer: even-odd pairs
    for q1, q2 in zip(qubits[0::2], qubits[1::2]):
        qc = qc.compose(conv_circuit(params[param_index : (param_index + 3)]), [q1, q2])
        qc.barrier()
        param_index += 3
    
    # Second layer: shifted pairs (only for more than 2 qubits)
    if num_qubits > 2:
        for q1, q2 in zip(qubits[1::2], qubits[2::2] + [0]):
            if param_index + 3 <= len(params):  # Safety check
                qc = qc.compose(conv_circuit(params[param_index : (param_index + 3)]), [q1, q2])
                qc.barrier()
                param_index += 3
...
for run in range(num_runs):
    ...
    if run == 0:  # Only create these once
        # Create custom feature map that maps 8 features to 4 qubits
        # We'll encode pairs of features on each qubit
        feature_map = QuantumCircuit(4)
        feature_params = ParameterVector('x', 8)  # 8 input features
        
        # Encode pairs of features on each qubit
        # Qubit 0: features 0,1
        # Qubit 1: features 2,3  
        # Qubit 2: features 4,5
        # Qubit 3: features 6,7
        for i in range(4):
            feature_map.ry(feature_params[2*i], i)      # First feature of the pair
            feature_map.rz(feature_params[2*i+1], i)    # Second feature of the pair
        
        ansatz = QuantumCircuit(4, name="Ansatz")  # Use 4 qubits instead of 8
        
        # Simplified ansatz for 4 qubits
        ...
        # Second Convolutional Layer (on the remaining 2 qubits after pooling)
        ansatz.compose(conv_layer(2, "c2"), list(range(2, 4)), inplace=True)
        
        # Second Pooling Layer (final pooling: 2 qubits → 1 qubit)
        ansatz.compose(pool_layer([0], [1], "p2"), list(range(2, 4)), inplace=True)
        
        # Combining the feature map and ansatz
        circuit = QuantumCircuit(4)  # Use 4 qubits instead of 8
        circuit.compose(feature_map, range(4), inplace=True)
        circuit.compose(ansatz, range(4), inplace=True)
        
        observable = SparsePauliOp.from_list([("Z" + "I" * 3, 1)])  # Measure first qubit of 4
        
        # we decompose the circuit for the QNN to avoid additional data copying
        qnn = EstimatorQNN(
            circuit=circuit.decompose(),
            observables=observable,
            input_params=feature_params,  # Use the 8 feature parameters
...
filename = f'qcnn_8features_4qubits_100_runs_results_{int(time.time())}.csv'
...
plt.tight_layout()
plot_filename = f'qcnn_8features_4qubits_100_runs_plot_{int(time.time())}.png'
