...
for run in range(num_runs):
    ...
    if run == 0:  # Only create these once
        print("  Building multi-layer quantum circuit...")
        feature_map = ZFeatureMap(8)
        
        ansatz = QuantumCircuit(8, name="Multi-Layer Ansatz")
        
        # First Convolutional Layer (8 qubits)
        print("    Adding conv layer 1 (8 qubits)...")
        ...
        # Second Convolutional Layer (8 qubits) - DUPLICATE
        print("    Adding conv layer 2 (8 qubits) - duplicate...")
        ...
        print("    Adding conv layer 3 (4 qubits)...")
        ...
        # Fourth Convolutional Layer (4 qubits) - DUPLICATE
        print("    Adding conv layer 4 (4 qubits) - duplicate...")
        ...
        # Fifth Convolutional Layer (2 qubits)
        print("    Adding conv layer 5 (2 qubits)...")
        ...
        # Sixth Convolutional Layer (2 qubits) - DUPLICATE
        print("    Adding conv layer 6 (2 qubits) - duplicate...")
        ...
        print(f"    Multi-layer architecture: 8->8->4->4->2->2 qubits")
...
filename = f'qcnn_multilayer_8qubit_{num_runs}runs_results_{int(time.time())}.csv'
...
plt.title('Distribution of Accuracies\n(Multi-Layer: 8->8->4->4->2->2 qubits)')
plt.legend()
...
plt.ylabel('Accuracy (%)')
plt.title('Accuracy vs Run Number\n(Duplicated Layer Architecture)')
plt.legend()

plt.tight_layout()
plot_filename = f'qcnn_multilayer_8qubit_{num_runs}runs_plot_{int(time.time())}.png'
plt.savefig(plot_filename, dpi=300, bbox_inches='tight')
print(f"Plot saved to: {plot_filename}")
print(f"\n🏗️  DUPLICATED MULTI-LAYER ARCHITECTURE SUMMARY:")
print(f"   8 qubits -> Conv1 -> Pool1 -> Conv2 -> Pool2")
print(f"   4 qubits -> Conv3 -> Pool3 -> Conv4 -> Pool4")  
print(f"   2 qubits -> Conv5 -> Pool5 -> Conv6 -> Pool6")
print(f"   Total layers: 6 Conv + 6 Pool = 12 layers")
print(f"   Each qubit level has 2x conv/pool layers for deeper learning")
plt.show()