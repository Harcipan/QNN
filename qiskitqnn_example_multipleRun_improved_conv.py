"""
Qiskit example QNN running with HARDWARE-EFFICIENT CONVOLUTION CIRCUITS
Using hardware-efficient conv circuits optimized for NISQ devices from improved_layers.py
"""
...
# Using hardware-efficient convolution circuit for NISQ devices
def conv_circuit(params):
    """Hardware-efficient ansatz style convolution - good for NISQ devices"""
    return hardware_efficient_conv_circuit(params)

# Alternative improved version (not used in this run)
def conv_circuit_improved(params):
    """Enhanced convolution circuit with more expressivity"""
    return improved_conv_circuit(params)

# Let's use the hardware-efficient conv circuit
params = ParameterVector("θ", length=6)  # 6 parameters for hardware-efficient
circuit = conv_circuit(params)
print("Hardware-efficient convolution circuit:")
print(circuit.draw())
...
print("Hardware-efficient conv layer:")
...
# Run simulation with hardware-efficient circuits
num_runs = 1
print(f"Starting {num_runs} simulation runs with HARDWARE-EFFICIENT CONVOLUTION CIRCUITS...")
print("Hardware-efficient circuits use 6 parameters per conv gate optimized for NISQ devices")
...
for run in range(num_runs):
    ...
    if run == 0:  # Only create these once
        print("  Building hardware-efficient quantum circuit...")
        feature_map = ZFeatureMap(8)
        
        ansatz = QuantumCircuit(8, name="Hardware-Efficient Ansatz")
        
        # First Convolutional Layer - Hardware-Efficient
        print("    Adding hardware-efficient conv layer 1...")
        ...
        # Second Convolutional Layer - Hardware-Efficient
        print("    Adding hardware-efficient conv layer 2...")
        ...
        # Third Convolutional Layer - Hardware-Efficient
        print("    Adding hardware-efficient conv layer 3...")
...
filename = f'qcnn_hardware_eff_{num_runs}runs_results_{int(time.time())}.csv'
...
plt.title('Distribution of Accuracies\n(Hardware-Efficient Conv Circuits)')
plt.legend()
...
plt.tight_layout()
plot_filename = f'qcnn_hardware_eff_{num_runs}runs_plot_{int(time.time())}.png'
plt.savefig(plot_filename, dpi=300, bbox_inches='tight')
print(f"Enhanced plot saved to: {plot_filename}")

# Print parameter count comparison
print(f"\n{'='*60}")
print(f"HARDWARE-EFFICIENT CIRCUIT ANALYSIS")
print(f"{'='*60}")
print(f"Hardware-Efficient Convolution: 6 parameters per gate (vs 3 original)")
print(f"Enhanced Pooling: 4 parameters per gate (vs 3 original)")
print(f"Hardware-efficient design optimized for NISQ devices with limited connectivity")
print(f"Should provide good performance with noise resilience")
